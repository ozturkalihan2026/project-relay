...
class FlutterClientContractTests(unittest.TestCase):
    def test_client_package_declares_expected_release(self) -> None:
        ...
        self.assertIn("version: 0.8.23+91", pubspec)
        ...
        self.assertIn("PROJECT RELAY • v0.8.23", manual)
    ...
    def test_v0818_fixed_header_credit_statistics_and_battle_vfx_contract(self) -> None:
        status = (CLIENT / "lib/src/widgets/player_status_bar.dart").read_text(encoding="utf-8")
        credit = (CLIENT / "lib/src/widgets/circuit_credit_icon.dart").read_text(encoding="utf-8")
        notice = (CLIENT / "lib/src/widgets/relay_notice.dart").read_text(encoding="utf-8")
        replay = (CLIENT / "lib/src/screens/replay_screen.dart").read_text(encoding="utf-8")
        attack = (CLIENT / "lib/src/widgets/replay_attack_overlay.dart").read_text(encoding="utf-8")
        season = (CLIENT / "lib/src/screens/season_screen.dart").read_text(encoding="utf-8")
        play = (CLIENT / "lib/src/screens/play_mode_screen.dart").read_text(encoding="utf-8")
        play_backdrop = (CLIENT / "lib/src/widgets/play_mode_backdrop.dart").read_text(encoding="utf-8")
        collection = (CLIENT / "lib/src/screens/collection_screen.dart").read_text(encoding="utf-8")
        season_api = (ROOT / "relay_api/season.py").read_text(encoding="utf-8")
        ...
        self.assertNotIn("Icons.toll_outlined", collection)
        self.assertIn("CircuitCreditGlyph", notice)
        self.assertIn("CircuitCreditGlyph", collection)
        self.assertIn("appBar: AppBar(", replay)
        self.assertIn("leading: const AppHeaderProfile()", replay)
        self.assertIn("actions: const [AppHeaderActions(), SizedBox(width: 8)]", replay)
        self.assertIn("event.type == 'destroyed'", replay)
        self.assertIn("event.type == 'repair'", replay)
        self.assertIn("RelayEmblem", season)
        self.assertIn("_LeaderboardRankBadge", season)
        self.assertIn("Icons.workspace_premium_rounded", season)
        self.assertIn('title=f"SEZON {starts_at.year}.{starts_at.month:02d}"', season_api)
        self.assertIn("PlayModeBackdrop", play)
        self.assertIn("class PlayModeBackdrop", play_backdrop)
        self.assertIn("DEVRE DIŞI", attack)
        self.assertIn("ENERJİ YETERSİZ", attack)
        self.assertIn("MaskFilter.blur", attack)
        self.assertIn("_drawImpactBurst", attack)

    def test_v0819_layout_header_dialog_scroll_and_combat_feedback_contract(self) -> None:
        header = (CLIENT / "lib/src/widgets/app_header_actions.dart").read_text(encoding="utf-8")
        ...
        dialog = (CLIENT / "lib/src/widgets/relay_dialog.dart").read_text(encoding="utf-8")
        ...
        attack = (CLIENT / "lib/src/widgets/replay_attack_overlay.dart").read_text(encoding="utf-8")
        replay_game = (CLIENT / "lib/src/game/replay_game.dart").read_text(encoding="utf-8")
        ...
        self.assertNotIn("ANA MERKEZ VE MENÜ DÜZENİ", main_menu)
        self.assertIn("_drawShieldEvent", attack)
        self.assertIn("_drawRepairEvent", attack)
        self.assertIn("_drawDestroyedEvent", attack)
        self.assertIn("_drawTargetHitFrame", attack)
        self.assertIn("quadraticBezierTo", attack)
        self.assertIn("Savaş alanını boş bir ızgara olmaktan çıkaran", replay_game)
        self.assertIn("criticalPulse", replay_game)
    ...
    def test_v084_profile_clan_header_and_career_preparation_contract(self) -> None:
        ...
        self.assertIn("PROJECT RELAY • v0.8.23", widget_test)
        self.assertIn("main-menu-online", widget_test)
        self.assertIn("main-menu-career", widget_test)
        self.assertIn("career-path-node-5", widget_test)
    ...
    def test_v0813_arcade_motion_contract(self) -> None:
        ...
        self.assertIn("class RelayEmblem", emblem)
        self.assertIn("portOffsets", emblem)
        self.assertIn("RelayEmblemPainter.drawEmblem", background)
        self.assertIn("index % ports.length", background)
        self.assertIn("RelayEmblem(", main_menu)
...
def test_v0820_rev1_analyzer_regressions():
    ...
    assert "version: 0.8.23+91" in pubspec
...
def test_v0821_rev2_25d_combat_contract():
    replay_game = (CLIENT / 'lib' / 'src' / 'game' / 'replay_game.dart').read_text(encoding='utf-8')
    overlay = (CLIENT / 'lib' / 'src' / 'widgets' / 'replay_attack_overlay.dart').read_text(encoding='utf-8')
    director = (CLIENT / 'lib' / 'src' / 'game' / 'battle_visual_director.dart').read_text(encoding='utf-8')
    camera = (CLIENT / 'lib' / 'src' / 'widgets' / 'battle_camera_rig.dart').read_text(encoding='utf-8')
    assert 'perspectiveShear' in replay_game
    assert '_drawBoardPlatform' in replay_game
...
def test_v0821_rev4_cinematic_final_contract():
    overlay = (CLIENT / 'lib' / 'src' / 'widgets' / 'replay_attack_overlay.dart').read_text(encoding='utf-8')
    sound = (CLIENT / 'lib' / 'src' / 'game' / 'event_sound_player.dart').read_text(encoding='utf-8')
    camera = (CLIENT / 'lib' / 'src' / 'widgets' / 'battle_camera_rig.dart').read_text(encoding='utf-8')
    assert 'JENERATÖR AÇIĞA ÇIKTI' in overlay
    assert 'ÇEKİRDEK AÇIKTA' in overlay
    assert 'ÇEKİRDEK ÇÖKTÜ' in overlay
